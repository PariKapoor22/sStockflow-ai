...
from .config import AUTH_DISABLED_FOR_LOCAL, DEV_TENANT_ID, SUPABASE_JWT_AUDIENCE, SUPABASE_JWT_ISSUER, SUPABASE_URL
...
def get_auth_context(request: Request) -> AuthContext:
    ...
    try:
        ...
        claims = decode(token, key.key, algorithms=["RS256"], audience=SUPABASE_JWT_AUDIENCE, issuer=SUPABASE_JWT_ISSUER)
    except Exception as exc:
        raise HTTPException(status_code=401, detail="Invalid access token") from exc
    app_metadata = claims.get("app_metadata") or {}
    tenant_id = app_metadata.get("tenant_id") or claims.get("tenant_id")
    if not tenant_id:
