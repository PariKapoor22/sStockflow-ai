...
class ForecastControllerTest(
    ...
    fun `forecast run evaluates models and persists daily values`() {
        ...
        mockMvc.get("/api/v1/forecasts/latest") {
            ...
            jsonPath("$[0].mase") { exists() }
            jsonPath("$[0].rmsse") { exists() }
            jsonPath("$[0].selectedAggregation") { exists() }
            jsonPath("$[0].averageDemandInterval") { exists() }
            jsonPath("$[0].coefficientVariationSquared") { exists() }
        }
        ...
        mockMvc.get("/api/v1/forecasts/configuration") {
            ...
            status { isOk() }
            jsonPath("$.enabledModels.length()") { value(10) }
            jsonPath("$.outlierTreatmentEnabled") { value(true) }
            jsonPath("$.weeklyAggregationEnabled") { value(true) }
            jsonPath("$.minimumNonZeroObservations") { value(8) }
        }

        mockMvc.get("/api/v1/forecasts/diagnostics") {
            header("X-Tenant-ID", "TEN-ACME-PHARMA")
        }.andExpect {
            status { isOk() }
            jsonPath("$.length()") { value(1) }
            jsonPath("$[0].eligibilityStatus") { value("ELIGIBLE") }
            jsonPath("$[0].selectedModel") { exists() }
            jsonPath("$[0].selectedAggregation") { exists() }
            jsonPath("$[0].averageDemandInterval") { exists() }
        }
        ...
        mockMvc.get("/api/v1/forecasts/calibration-summary") {
            header("X-Tenant-ID", "TEN-ACME-PHARMA")
        }.andExpect {
            status { isOk() }
            jsonPath("$.positionsAnalyzed") { value(1) }
            jsonPath("$.eligiblePositions") { value(1) }
            jsonPath("$.averageSelectedWape") { exists() }
            jsonPath("$.averageMase") { exists() }
        }
    }


    @Test
    fun `ineligible sparse position is diagnosed instead of forced through a model`() {
        jdbcTemplate.update(
            """INSERT INTO retailer
               (retailer_id, tenant_id, retailer_name, retailer_type, warehouse_id, city, region, credit_days, active)
        ...
        val response = mockMvc.post("/api/v1/forecasts/runs") {
            ...
            jsonPath("$.positionsRequested") { value(1) }
            jsonPath("$.positionsProcessed") { value(0) }
            jsonPath("$.positionsFailed") { value(1) }
        }.andReturn().response.contentAsString

        val runId = Regex("\\\"forecastRunId\\\":\\\"([^\\\"]+)\\\"")
            .find(response)?.groupValues?.get(1) ?: error("forecastRunId missing")
        ...
        mockMvc.get("/api/v1/forecasts/diagnostics") {
            ...
            status { isOk() }
            jsonPath("$[0].eligibilityStatus") { value("INSUFFICIENT_NON_ZERO_DEMAND") }
            jsonPath("$[0].selectedModel") { doesNotExist() }
            jsonPath("$[0].reasonCodes") { isArray() }
        }
    }
