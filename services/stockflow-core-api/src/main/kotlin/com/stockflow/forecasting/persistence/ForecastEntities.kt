...
open class ForecastConfigurationEntity(
    ...
    @Column(name = "high_confidence_mase", nullable = false, precision = 7, scale = 3)
    open var highConfidenceMase: BigDecimal = BigDecimal("0.800"),

    @Column(name = "medium_confidence_mase", nullable = false, precision = 7, scale = 3)
    open var mediumConfidenceMase: BigDecimal = BigDecimal("1.500"),

    @Column(name = "minimum_non_zero_observations", nullable = false)
    open var minimumNonZeroObservations: Int = 8,

    @Column(name = "maximum_forecastable_cv_squared", nullable = false, precision = 9, scale = 4)
    open var maximumForecastableCvSquared: BigDecimal = BigDecimal("10.0000"),

    @Column(name = "weekly_aggregation_enabled", nullable = false)
    open var weeklyAggregationEnabled: Boolean = true,

    @Column(name = "outlier_treatment_enabled", nullable = false)
    open var outlierTreatmentEnabled: Boolean = true,

    @Column(name = "enabled_models", nullable = false, length = 400)
...
open class ForecastModelPerformanceEntity(
    ...
    @Enumerated(EnumType.STRING)
    @Column(name = "aggregation_level", nullable = false, length = 20)
    open var aggregationLevel: ForecastAggregationLevel = ForecastAggregationLevel.DAILY,
    ...
    open var eligibilityStatus: ForecastEligibilityStatus = ForecastEligibilityStatus.ELIGIBLE,
    ...
    @Column(name = "non_zero_observations", nullable = false)
    open var nonZeroObservations: Int = 0,

    @Column(name = "zero_demand_ratio", nullable = false, precision = 9, scale = 6)
    open var zeroDemandRatio: BigDecimal = BigDecimal.ZERO,

    @Column(name = "average_demand_interval", nullable = false, precision = 19, scale = 6)
    open var averageDemandInterval: BigDecimal = BigDecimal.ZERO,

    @Column(name = "coefficient_variation_squared", nullable = false, precision = 19, scale = 6)
    open var coefficientVariationSquared: BigDecimal = BigDecimal.ZERO,
    ...
    @Column(name = "mase", precision = 19, scale = 6)
    open var mase: BigDecimal? = null,

    @Column(name = "rmsse", precision = 19, scale = 6)
    open var rmsse: BigDecimal? = null,
...
enum class ForecastAggregationLevel {
    DAILY,
    WEEKLY
}

enum class ForecastEligibilityStatus {
    ELIGIBLE,
    LIMITED_HISTORY,
...
enum class ForecastDiagnosticReason {
    INTERMITTENT_DEMAND,
    LUMPY_DEMAND,
...
@Entity
@Table(name = "forecast_position_diagnostic")
open class ForecastPositionDiagnosticEntity(
    @Id
    @Column(name = "forecast_position_diagnostic_id", nullable = false)
    open var forecastPositionDiagnosticId: UUID = UUID.randomUUID(),
    ...
    @Enumerated(EnumType.STRING)
    @Column(name = "selected_aggregation", nullable = false, length = 20)
    open var selectedAggregation: ForecastAggregationLevel = ForecastAggregationLevel.DAILY,

    @Column(name = "history_observations", nullable = false)
    open var historyObservations: Int = 0,
    ...
    @Enumerated(EnumType.STRING)
    @Column(name = "selected_model", length = 60)
    open var selectedModel: ForecastModelCode? = null,

    @Column(name = "selected_wape", precision = 19, scale = 6)
    open var selectedWape: BigDecimal? = null,

    @Column(name = "selected_mase", precision = 19, scale = 6)
    open var selectedMase: BigDecimal? = null,

    @Column(name = "selected_rmsse", precision = 19, scale = 6)
    open var selectedRmsse: BigDecimal? = null,

    @Column(name = "best_daily_wape", precision = 19, scale = 6)
    open var bestDailyWape: BigDecimal? = null,

    @Column(name = "reason_codes", nullable = false, length = 1200)
    open var reasonCodes: String = "",
